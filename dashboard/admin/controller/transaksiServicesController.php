<?php
// admin/controller/transaksiServicesController.php
session_start();
...
if (isAjaxRequest()) {
    ...
    if ($request == 'get_transaksi_services') {
        try {
            ...
            $query = "SELECT 
                      ...
                      WHERE ts.status_transaksi_service = 'pending'
            ...
            if (count($transaksis) > 0) {
                ...
                <div class="alert alert-info text-center" role="alert">
                    <i class="fas fa-info-circle me-2"></i>Belum ada data transaksi service.
                </div>
                <?php
            }
    ...
    if ($request == 'get_transaksi_detail') {
        ...
        try {
            $query = "SELECT 
                      ...
                      JOIN tb_user u ON b.id_user = u.id_user
                      JOIN tb_pelanggan p ON u.id_user = p.id_user
                      WHERE ts.id_transaksi_service = ? AND ts.status_transaksi_service = 'pending'";
    ...
    // Confirm Transaksi Service (Change status from pending to completed)
    if ($request == 'confirm_transaksi') {
        $transaksi_id = $_POST['transaksi_id'] ?? 0;
        ...
        try {
            ...
            $check_query = "SELECT status_transaksi_service FROM tb_transaksi_service WHERE id_transaksi_service = ?";
            ...
            if (strtolower($transaksi_data['status_transaksi_service']) !== 'pending') {
                echo json_encode(['status' => 'error', 'message' => 'Transaksi ini sudah dikonfirmasi sebelumnya']);
                exit;
            }

            // Update status to completed
            $update_query = "UPDATE tb_transaksi_service SET status_transaksi_service = 'completed' WHERE id_transaksi_service = ?";
